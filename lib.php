...
function facetoface_user_signup($session, $facetoface, $course, $discountcode,
    ...
    // Deduct user module on Zebra if user has enough
    $resp = restoreOrDeductUserZebraModule('deduct', $userid);
    if (!$resp->data->success) {
        print_error('nomoduleleft', 'facetoface', $resp->data->message);
        return false;
    }
 ...
 * @return mixed
 */
function restoreOrDeductUserZebraModule($action, $userId) {
    global $DB;

    $secretFile = 'secret.txt';
    $secretOpen = fopen($secretFile, 'r') or die('Unable to open secret file');
    $token = fread($secretOpen, filesize($secretFile));
    fclose($secretOpen);
...
function facetoface_user_cancel($session, $userid=false, $forcecancel=false, &$errorstr=null, $cancelreason='') {
    ...
    if (facetoface_user_cancel_submission($session->id, $userid, $cancelreason)) {
        ...
        restoreOrDeductUserZebraModule('restore', $userid);
